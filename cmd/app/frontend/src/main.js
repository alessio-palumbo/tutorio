...
guideContent.addEventListener('click',async event=>{const link=event.target.closest('[data-source-path]');if(!link)return;try{await backend().OpenSourceFile(link.dataset.sourcePath,Number(link.dataset.sourcePage)||1)}catch(err){message.textContent=String(err)}})
