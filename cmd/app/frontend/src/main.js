...
app.innerHTML = `
  ...
  <main>
    <section class="create">
      ...
      <form id="compile-form"><label for="url">YouTube URL</label><div class="row"><input id="url" type="url" placeholder="https://youtube.com/watch?v=…" required><button>Compile guide</button></div></form><div class="import-row"><span>Or compile an existing TXT, SRT, or VTT file.</span><button class="quiet" id="import-transcript">Import file</button></div>
...
const localPageURL=(path,page)=>`file://${encodeURI(path)}#page=${Math.max(1,Number(page)||1)}`
const renderSourceReferences=(references,timestamps,sourceURI)=>{const values=asArray(references);if(!values.length)return renderTimestamps(timestamps,sourceURI);return `<div class="timestamps">${values.map(value=>{if(value.kind==='page'){const start=value.page_start||1;const end=value.page_end||start;const label=start===end?`Page ${start}`:`Pages ${start}–${end}`;return `<button data-source-url="${escapeHTML(localPageURL(sourceURI,start))}">${label}${value.label?` · ${escapeHTML(value.label)}`:''} ↗</button>`}const url=timestampURL(sourceURI,value.start_seconds);const label=`${formatTime(value.start_seconds)}${value.label?` · ${escapeHTML(value.label)}`:''}`;return url?`<button data-source-url="${escapeHTML(url)}">${label} ↗</button>`:`<span>${label}</span>`}).join('')}</div>`}
...
function renderGuide(guide) {
  ...
  const guideSections=indices.map((sectionIndex,position)=>{const source=currentSections.find(section=>section.index===sectionIndex);const indexedSteps=(guide.steps||[]).map((step,index)=>({step,index})).filter(item=>(item.step.source_segment??0)===sectionIndex);const deepDive=(guide.deep_dives||[]).find(item=>item.source_segment===sectionIndex);const title=source?.guide?.title||`Section ${position+1}`;const timestamps=indexedSteps.flatMap(item=>item.step.timestamps||[]);const start=timestamps.length?Math.min(...timestamps.map(item=>Number(item.start_seconds)||0)):0;const end=timestamps.length?Math.max(...timestamps.map(item=>Number(item.end_seconds)||0)):start;const actions=source?`<div class="section-actions"><button class="quiet" data-delve-section="${sectionIndex}">${deepDive?'Refresh deep dive':'Delve deeper'}</button><button class="quiet" data-regenerate-section="${sectionIndex}">Regenerate</button></div>`:'';const dive=deepDive?`<article class="deep-dive"><span class="eyebrow">DEEP DIVE · ${escapeHTML(deepDive.model||'LOCAL MODEL')}</span><h3>${escapeHTML(deepDive.title)}</h3><p>${escapeHTML(deepDive.explanation)}</p>${renderList('Key points',deepDive.key_points)}${renderList('Examples',deepDive.examples)}${renderList('Caveats',deepDive.caveats)}${renderList('Source evidence',deepDive.evidence)}</article>`:'';return `<section class="guide-section"><div class="guide-section-head"><div><span class="eyebrow">SOURCE SECTION ${position+1}</span><h2>${escapeHTML(title)}</h2><span class="section-range">${formatTime(start)}–${formatTime(end)}</span></div>${actions}</div><div class="steps">${indexedSteps.map(item=>renderStep(item.step,item.index)).join('')}</div>${dive}</section>`}).join('')
...
document.querySelector('#import-transcript').addEventListener('click',async event=>{const button=event.currentTarget;button.disabled=true;try{const job=await backend().SelectAndQueueTranscript();if(job?.id){message.textContent='Transcript compilation queued.';progress.hidden=false;progress.querySelector('div').style.width='4%';await loadJobs()}}catch(err){message.textContent=String(err)}finally{button.disabled=false}})
