...
const renderTimestamps = (values = []) => values.length ? `<div class="timestamps">${values.map(value => `<span>${formatTime(value.start_seconds)}${value.label ? ` · ${escapeHTML(value.label)}` : ''}</span>`).join('')}</div>` : ''
...
function renderGuide(guide) {
  ...
  const steps = (guide.steps || []).map(step => `<article class="step"><div class="step-number">${step.number}</div><div><h3>${escapeHTML(step.title)}</h3><p>${escapeHTML(step.explanation)}</p>${renderTimestamps(step.timestamps)}${renderList('Actions', step.actions)}${step.commands?.length ? `<h4>Commands</h4>${step.commands.map(command => `<pre><code>${escapeHTML(command)}</code></pre>`).join('')}` : ''}${renderList('Warnings', step.warnings)}</div></article>`).join('')
  ...
  guideContent.innerHTML = `<article class="guide"><span class="eyebrow">${escapeHTML(guide.source_type)} GUIDE</span><h1>${escapeHTML(guide.title)}</h1><p class="lead">${escapeHTML(guide.overview)}</p><section class="outcome"><h2>Final outcome</h2><p>${escapeHTML(guide.final_outcome)}</p></section>${renderList('Prerequisites', guide.prerequisites)}<section><h2>Step-by-step guide</h2><div class="steps">${steps}</div></section>${renderList('Important concepts', guide.important_concepts)}${commands}${shortcuts}${renderList('Warnings', guide.warnings)}${renderList('Common mistakes', guide.common_mistakes)}${renderList('Cheat sheet', cheatSheet)}${renderList('Appendix', guide.appendix)}<section><h2>Source timestamps</h2>${renderTimestamps(guide.source_timestamps)}</section></article>`
}
