...
function updateActiveRegenerationFeedback(){
  ...
  if(activeRegeneration&&!library.hidden){message.textContent=`Regenerating “${activeRegeneration.sectionTitle}” of “${activeRegeneration.guideTitle}” · ${elapsedLabel(Date.now()-activeRegeneration.startedAt)} elapsed`;progress.hidden=false;progress.classList.add('indeterminate')}
}
...
document.querySelector('#compile-form').addEventListener('submit', async event => {
  event.preventDefault(); const button = event.currentTarget.querySelector('button'); button.disabled = true; progress.hidden = false; progress.classList.remove('indeterminate'); progress.querySelector('div').style.width = '4%'; message.textContent = 'Adding compilation to the local queue…'
...
document.querySelector('#import-file').addEventListener('click',async event=>{const button=event.currentTarget;button.disabled=true;try{const job=await backend().SelectAndQueueFile();if(job?.id){message.textContent='File compilation queued.';progress.hidden=false;progress.classList.remove('indeterminate');progress.querySelector('div').style.width='4%';await loadJobs()}}catch(err){message.textContent=String(err)}finally{button.disabled=false}})
...
window.runtime?.EventsOn?.('pipeline:progress', update => {
  ...
  if(sectionRegeneration&&activeRegeneration&&activeRegeneration.guideID===update.guide_id&&activeRegeneration.index===Number(update.section_index))updateActiveRegenerationFeedback();else if(!sectionRegeneration)message.textContent=update.message
