...
document.querySelector('#export-guide').addEventListener('click',async()=>{if(!currentGuide)return;try{const path=await backend().ExportMarkdown(currentGuide.id);if(path)showMessage(`Exported to ${path}`,{dismissible:true})}catch(err){showMessage(err,{dismissible:true})}})
