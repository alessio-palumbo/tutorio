...
func (s *JobStore) CompleteSegment(ctx context.Context, value jobs.Segment) error {
	...
	_, err = s.db.ExecContext(ctx, `INSERT INTO job_segments(job_id,segment_index,transcript_json,guide_json,status,model,prompt_tokens,output_tokens,duration_ms) VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(job_id,segment_index) DO UPDATE SET transcript_json=excluded.transcript_json,guide_json=excluded.guide_json,status=excluded.status,model=excluded.model,prompt_tokens=excluded.prompt_tokens,output_tokens=excluded.output_tokens,duration_ms=excluded.duration_ms`, value.JobID, value.Index, transcriptJSON, guideJSON, value.Status, value.Model, value.PromptTokens, value.OutputTokens, value.DurationMilliseconds)
	return err
}
...
func (s *JobStore) Segments(ctx context.Context, jobID string) ([]jobs.Segment, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT segment_index,transcript_json,guide_json,status,model,prompt_tokens,output_tokens,duration_ms FROM job_segments WHERE job_id=? ORDER BY segment_index`, jobID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var guideJSON []byte
		if err := rows.Scan(&item.Index, &transcriptJSON, &guideJSON, &item.Status, &item.Model, &item.PromptTokens, &item.OutputTokens, &item.DurationMilliseconds); err != nil {
			return nil, err
		}
		item.JobID = jobID
