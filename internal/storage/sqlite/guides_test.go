...
func TestJobStorePersistsSegmentsAndMetrics(t *testing.T) {
	...
	if err = store.CompleteSegment(context.Background(), jobs.Segment{JobID: job.ID, Index: 0, Transcript: segments[0], Guide: guide.Guide{Title: "Part"}, Status: jobs.StatusCompleted, Model: "test", PromptTokens: 10, OutputTokens: 20, DurationMilliseconds: 30, PromptDurationMilliseconds: 11, OutputDurationMilliseconds: 17, RawResponse: `{"title":"Part"}`}); err != nil {
		t.Fatal(err)
	}
	...
	if len(stored) != 1 || stored[0].OutputTokens != 20 || stored[0].PromptDurationMilliseconds != 11 || stored[0].OutputDurationMilliseconds != 17 || stored[0].Guide.Title != "Part" || stored[0].RawResponse != `{"title":"Part"}` {
